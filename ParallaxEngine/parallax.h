...
void layer_render(layer_t *lr, SDL_Surface *screen, SDL_Rect *rect)
{
	...
	if(rect)
	{
		...
		// make sure local clip inside of screen clip.
	...
	mx_start = mx;
	pos.h = TILE_H;
	for(pos.y = screen->clip_rect.y - fine_y; pos.y < max_y; pos.y += TILE_H)
	{
		mx = mx_start;
		my %= MAP_H;
		for(pos.x = screen->clip_rect.x - fine_x; pos.x < max_x; )
		{
			int	tile, tm, run_w;
			mx %= MAP_W;
			tile = (*lr->map)[my][mx];
			...
			if(detect_runs)
			  while(pos.x + run_w*TILE_W < max_x)
			  {
				...
				if(tm != tt)
				{
					// i found a bug(99% sure). if tm is keyed and there's empty tiles, it 
					// doesn't do a run. just renders one keyed tile and continues on with
					// the rest of the render routine.

					// if tm is empty it and there's keyed tiles, it does do a run. I wonder
					// if this makes a detectalble difference in framerate if I fix
					// this bug?
					// AD_TODO: profile this change

					//if ( tm == TM_KEYED && tt == TM_EMPTY )
					//	__asm nop;
					//else
						break;
				}
...
class ParallaxDemo : public SDL_App
{
	SDL_Surface	*screen;
...
public:
	...
	bool Init() override
	{
		...
		// AD_TODO: profile different flags
		...
		if(num_of_layers > 1)
		{
			...
			for(i = 1; i < num_of_layers - 1; ++i)
			{
				...
				if( (i & 1) && use_planets) // great way to test for odd number
				{	
			...
			layers[i].name = "layer" + std::to_string(num_of_layers-1);
